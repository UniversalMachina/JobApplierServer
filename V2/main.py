...
def ask_mcq():
    ...
    try:
        ...
        try:
            ...
            if not user_answer or user_answer < 1:
                user_answer = 1
    print(user_answer)
    if not user_answer or user_answer < 1:
        user_answer=1
...
def ask_question():
    ...
    # check if 'cover letter' is in the question
        if 'cover letter' in question['question'].lower():
            ...
            if option_picker == 1:
                user_answer = generate_text(
                    ...
                    f"Answer this question from the POV of Eric Bennett. If the resume does not have the relevant information or experience make up an answer. For personal information such as a zip code OR salary expectations OR social media(linkedin or twitter) that's not included respond prefer not to answer. Keep the answer as short and concise as possible\n")
                print(user_answer)
            elif option_picker == 2:
                user_answer = generate_text(
                    f"Here is the resume:\n {resume}\n"
                    f"Here is the question or information that is being inquired:\n {question['question']}\n"
                    f"Answer this question from the POV of Eric Bennett. If the resume does not have the relevant information make something up. Keep the answer short and concise. Do not use hashtags\n")
                print(user_answer)
            else:
        ...
        # Example usage
        user_answer=str(remove_phrase(user_answer, 'As Eric Bennett, '))
        user_answer = str(remove_phrase(user_answer, 'POV of Eric Bennett, '))
        return user_answer
...
def ask_number_question():
    ...
    user_answer = generate_text(
        f"Here is the resume:\n {resume}\n"
        f"here is the question or information that is being inquired:\n {question['question']}\n"
        f"Answer this questions from the POV of Eric Bennett. If the resume does not have the relevant information or experience make up an answer. Return only a number answer to the question and nothing else. \n1\nis a valid answer\n1 . Fluent in English\nis an invalid answer")
    print(user_answer)
