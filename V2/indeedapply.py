...
def ask_mcq():
    ...
    user_answer = generate_text(
        f"Answer these questions from the pov of the ideal candidate with a high skill level for a job living in Canada. They do not require a visa."
...
def check_words(input_string):
    ...
    #     return True
    if 'gpa'  in lower_string:
        return True
    else:
        return False

# Test the function
def ask_number_question_ori(question):
...
def ask_question():
    ...
    # check if 'cover letter' is in the question
        if 'cover letter' in question['question'].lower():
            ...
            user_answer=ask_number_question_ori(question['question'])
            return user_answer
            ...
            if option_picker == 1:
                user_answer = generate_text(
                    f"Here is the resume:\n {resume}\nrace{race}gender{gender}"
                    f"Here is the question or information that is being inquired:\n {question['question']}\n"
                    f"Answer this question from the POV of {name}. If the resume does not have the relevant information or experience make up an answer. For personal information such as a zip code/street address OR salary expectations OR social media(Linkedin or Twitter) that's not included respond prefer not to answer. Keep the answer as short and concise as possible, (bellow 140 characters)\n")
                print(user_answer)
            elif option_picker == 2:
                user_answer = generate_text(
                    f"Here is the resume:\n {resume}\nrace{race}gender{gender}"
                    f"Here is the question or information that is being inquired:\n {question['question']}\n"
                    f"Answer this question from the POV of {name}. If the resume does not have the relevant information make something up. Keep the answer short and concise (bellow 140 characters). Do not use hashtags\n")
                print(user_answer)
            else:
...
def ask_number_question():
    ...
    user_answer = generate_text(
        f"Here is the resume:\n {resume}\n"
        f"here is the question or information that is being inquired:\n {question['question']}\n"
        f"Answer this questions from the POV of {name}. If the resume does not have the relevant information or experience make up an answer. Assume they are knowledgeable and have a experience in most things. You do not require a visa. Return only a number answer to the question and nothing else. \n1\nis a valid answer\n1 . Fluent in English\nis an invalid answer")
    print(user_answer)
